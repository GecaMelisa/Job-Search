...
import './jobs.css';
import { Box, Button, FormControl, InputAdornment, InputLabel, Link, Menu, MenuItem, OutlinedInput, Select, Typography, styled } from '@mui/material';
  ...
  const handleJobTypeChange = async (e: any) => {
    ...
    /* useEffect(() => {
      const fetchData = async () => {
        const response = await fetch(`http://localhost:8080/api/jobs/type/${jobType}`);
    ...
    const fetchData = async () => {
      try {
        ...
        let jobsConfig = {
          ...
          url:`http://localhost:8080/api/jobs/jobsWithPaginationAndFiltering/{jobType}?offset=${page * pageSize}&pageSize=${pageSize}&field=${searchTerm}&jobType=${jobType}`,
         //url: `http://localhost:8080/api/jobs/type/${jobType.toUpperCase()}`, 
    ...
    const fetchDataType = async () => {
      try {
        ...
        let jobsConfig = {
          method: 'get',
          maxBodyLength: Infinity,
          url: `http://localhost:8080/api/jobs/getTypes`, 
      ...
      <form action="">
        <FormContainer>
          <SearchContainer>
            ...
            <Select value={jobType} onChange={handleJobTypeChange} style={{ width: '200px', border: '2px solid #4e66a2', height: '40px' }}>
              {!loading && jobType && jobType.map((jType, index) => (
...
{/*
    ...
    {!loading && allJobs && allJobs.map((job, index) => (
     ...
     <Typography variant="body1" sx={{ mx: 2}}>
      ...
      <Button onClick={() => { setPage(page + 1) }} disabled={(page * pageSize) + allJobs.length === totalCount}>Next Page</Button>
